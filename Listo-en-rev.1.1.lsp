(defun EZA ()
  ...
  ;; Display a message asking the user to press Enter after finishing(defun EZA ()
  ...
  (prompt "Press Enter after finish")
  ...
  (if (or(equal scaledim "")(equal scaledim nil))(setq scaledim 1))
  ;; Divide AddSum by the scale number entered by the user
  (setq AddSum (/ AddSum scaledim))
      ...
      (if (setq eset (ssget))
        (progn
          ...
          (while (< cntr (sslength eset))
            ...
            (setq totalLen (+ totalLen (/ len scaledim)))
            (setq cntr (+ cntr 1))
          )
        )
      )
...
;========================================================================================================================
(defun trim-entities-outside-rectangle (rect)
  (vl-load-com)
  (setq doc (vla-get-activedocument (vlax-get-acad-object)))
  (setq mspace (vla-get-modelspace doc))
  
  (setq rectMinPt (vlax-safearray->list (vlax-variant-value (vlax-get rect 'Coordinates))))
  (setq rectMaxPt (vlax-safearray->list (vlax-variant-value (vla-get-endpoint rect 1))))
  
  (setq selection (vla-get-selectionset doc))
  (vla-clear selection)
  
  (vla-selectall mspace)
  (setq allEnts (vla-get-activeselectionset mspace))
  (vla-clearselection allEnts)
  
  (vla-select mspace '(:vlax-false :vlax-true) rect)
  (vla-add allEnts (vla-get-activeselectionset mspace))
  
  (vla-select mspace '(:vlax-true :vlax-false) rect)
  (vla-add allEnts (vla-get-activeselectionset mspace))
  
  (vla-select mspace '(:vlax-true :vlax-true) rect)
  (vla-add allEnts (vla-get-activeselectionset mspace))
  
  (setq selCount (vla-get-count allEnts))
  
  (if (> selCount 0)
      (progn
        (vla-copy allEnts selection)
        (command "-_.erase" selection "")
        (setq rect (vla-addrectangle mspace rectMinPt rectMaxPt))
        (vla-update allEnts)
        (princ "\nEntities trimmed and deleted outside the rectangle.")
      )
      (princ "\nNo entities found outside the rectangle.")
  )
)
(defun point-outside-rectangle (point rectMinPt rectMaxPt)
  ; Check if the point is outside the rectangle boundaries
  (or (< (car point) (car rectMinPt))
      (< (cadr point) (cadr rectMinPt))
      (> (car point) (car rectMaxPt))
      (> (cadr point) (cadr rectMaxPt))
  )
)
...
;;=======================================================================================================================
(defun listBlock (/ ss name ref refs insPt table row)
  (vl-load-com)
  (or *acdoc*
  ...
  (or *blocks*
      ...
      (setq ss (ssget "_X" '((0 . "INSERT")(8 . "steels"))))
  ...
  (if ss
	...
	(if (not (vlax-property-available-p x 'Path))
      ...
      (vla-SetTextHeight table 7 5)
      (vla-put-VertCellMargin table 4.0)
      (vla-put-TitleSuppressed table :vlax-false)
      (vla-put-HeaderSuppressed table :vlax-false)
      (vla-setText table 0 0 "Shape of Rebars")
      (vla-setText table 1 0 "POSE NUMBER")
      (vla-setText table 1 1 "SHAPE")
...
(defun tbl ()
  ...
  (setq weight-list  '(("Ø8"  "0.617") ("Ø10"  "0.617") ("Ø12"  "0.888") ("Ø14"  "1.21") ("Ø16"  "1.58") ("Ø18" "2.00") ("Ø20" "2.47") ("Ø22" "2.98") ("Ø25" "3.85") ("Ø28"  "4.83") ("Ø30"  "5.55") ("Ø32"  "6.31")))
    (foreach line lines
      ...
      (if s
        ...
        (initget (+ 2 4))
        (if (not (setq totalqty (getreal(strcat "Total QTY for pose: " pose "and QTY:" totalle ))))
          (setq totalqty 1)
        )
        
        (setq unit-weight (cdr (assoc diame weight-list)))
        (setq totalWeigth (* totalqty (atof numbe) (atof leng) (atof  (car unit-weight))))
        (setq total-Lnumber (* totalqty (atof numbe) (atof leng)))
        (setq sumw (cons  totalWeigth sumw))
        (setq tab (list first pose numbe diame ate leng totalle (rtos totalqty 2 0) (rtos total-Lnumber 2 2) (rtos totalWeigth 2 2) 8th))
        (setq tbldata (append (list tab) tbldata))
        (setq sum (cons diame  total-Lnumber ))
  ...
  (foreach a lis
     ...
     (setq fname (getstring "first column:<Object> "))
        ...
        (setq seventh (getstring "7th column:<SubTotal(m)> "))
        (setq eigthth (getstring "8th column:<Total(QTY)> "))
        (setq nineth (getstring "9th column:<Total(m)> "))
        (setq tenth (getstring "10th column:<Total(kg)> "))
        (setq eleventh (getstring "11th column:<Position> "))
        ...
        (if (or(equal seventh "")(equal seventh nil))(setq seventh "SubTotal(m)" ))
        (if (or(equal eigthth "")(equal eigthth nil))(setq eigthth "Total(QTY)" ))
        (if (or(equal nineth "")(equal nineth nil))(setq nineth "Total(m)" ))
        (if (or(equal tenth "")(equal tenth nil))(setq tenth "Total(kg)" ))
        (if (or(equal eleventh "")(equal eleventh nil))(setq eleventh "Position" ))
        (if (or(equal table-name "")(equal table-name nil))(setq table-name "Listofer" ))
        (setq column-names (list fname sname thirth fourth fifth sixth seventh eigthth nineth tenth eleventh ))
        (setq Totalcolumn-names (list fourth nineth "Unit-Weigth(kg/m)" tenth))
        (vl-sort tbldata
             (function (lambda (e1 e2)
                         (< (atof(cadr  e1))) (atof (cadr e2)))))
        ;(setq tbldata (reverse tbldata))
...
;;=======================================================================================================================
;;======Delete shapes Drawings if not acceptable====================
;;=======================================================================================================================
(defun delete-and-redraw-shape (blockname)
  (vl-load-com)
  (setq doc (vla-get-activedocument (vlax-get-acad-object)))
  (setq mspace (vla-get-modelspace doc))

  ; Select the existing shape to delete
  (prompt "\nSelect the existing shape to delete: ")
  ...
  (if selection
      (progn
        (command-s "._erase" selection "")
        (prompt "\nShape deleted.")
        (princ)
      )
      (progn
        (prompt "\nNo shape selected. Nothing to delete.")
        (princ)
      )
  )
...
(defun create-stirrup-block (apt tool arz ht blockname)
  ...
  (setq linek2 (entlast))
  ; Call the trim-entities-outside-rectangle function
  (setq rec (entget rectangle))
  (setq end-pointr (cdr  (nth 20 rec)))
  (setq end-pointr1 (cdr (assoc 10 rec)))
  ;;(command-s "_.point"  end-pointr "" )
   (command "_.copy"  linek2 "" end-pointr1  end-pointr "")
   (setq linek1 (entlast))
  ...
  (setq acceptable (strcase (getstring "\nIs the shape acceptable now? ([Y/N]): ")))
  (while  (or (equal acceptable "N")(equal acceptable "n"))
     (delete-and-redraw-shape blockname)
    (vl-load-com)
      (setq doc (vla-get-activedocument (vlax-get-acad-object)))
      (setq mspace (vla-get-modelspace doc))
      (setq tt 0)
...
;;=======================================================================================================================
;;===========create a block for a Sanjaghi that is in both side in 135 Degree============================================
;;=======================================================================================================================

(defun create-135*Sanjaghi-block (aptSan toolSan khamSan htSan blocknameSan)
  (vl-load-com)
  (setq docSan (vla-get-activedocument (vlax-get-acad-object)))
  (setq mspaceSan (vla-get-modelspace docSan))
  (setq ttSan 0)
  (setq aaSan 0)
  (setq prevXSan (car aptSan))
  (setq prevYSan (cadr aptSan))
  (setq prevZSan (caddr aptSan))
  (setq offsetSan (* htSan 2))
  (setq newYSan (+ prevYSan offsetSan))
  (setq minValSan (min (/ aaSan 10) (/ ttSan 10)))
  (setq nxSan (+ prevXSan minValSan))
  (setq newstartXSan (list nxSan prevYSan prevZSan))
  (setq nySan (- prevYSan minValSan))
  (setq newstartYSan (list prevXSan nySan prevZSan))
  (setq newAptSan (list prevXSan newYSan prevZSan))
  (setq endpointSan (list (+ 0.50 prevXSan) newYSan prevZSan))
  (setq endpoint1San (list (+ prevXSan 0.16) (- newYSan 0.1) prevZSan))
  (setq endpoint2San (list (+ prevXSan 0.34 ) (- newYSan 0.1) prevZSan))
  (command-s "._line" newAptSan  endpointSan  "" "" "") ; Create line
  (setq line1San (entlast))
  (command-s "._line" newAptSan "<135"  endpoint1San  "" "" "") ; Create line
  (setq line2San (entlast))
  (command-s "._line" endpointSan "<45"  endpoint2San  "" "" "") ; Create line
  (setq line3San (entlast))
  (command "._fillet" line1San "Radius" 0.02 "" line2San) ; Fillet the lines
  (command "._fillet" line1San "Radius" 0.02 "" line3San) ; Fillet the lines
  (defun DtRSan (d) (* pi (/ d 180.0)))
  (setq endpoint4San (list (+ prevXSan 0.16) (- newYSan 0.12) prevZSan))
  (setq endpoint5San (list (+ prevXSan 0.25 ) (- newYSan 0.12) prevZSan))
  (setq midpointSan (polar endpoint4San (DtRSan 135) (/ (distance newAptSan endpoint1San) 2.0) ))
  (setq midpoint1San (polar endpoint5San (DtRSan 45) (/ (distance newAptSan endpoint1San) 2.0) ))

  (setq text1poSan  (list (+ 0.20 prevXSan) (+ newYSan (* (/ htSan 15) 2)) prevZSan) )

  (command-s "-style" "khayyam" "@Arial unicode MS" htSan 0.85 0 "N" "N")
  (setq text1San (rtos toolSan 2 2))
  (setq text2San (rtos khamSan 2 2))
  (setq text3San (rtos khamSan 2 2))
  (setq thetext1San (vla-AddText mspaceSan text1San (vlax-3d-point text1poSan) 0.05))
  (vla-put-layer thetext1San LayerName)
  (setq texts1San (entlast))
  (setq thetext2San (vla-AddText mspaceSan text2San (vlax-3d-point midpointSan) 0.03))
  (vla-put-layer thetext2San LayerName)
  (setq texts2San (entlast))
  (setq thetext3San (vla-AddText mspaceSan text3San (vlax-3d-point midpoint1San) 0.03))
  (vla-put-layer thetext3San LayerName)
  (setq texts3San (entlast))

  (vla-put-layer (vlax-ename->vla-object line1San) LayerName)
  (vla-put-layer (vlax-ename->vla-object line2San) LayerName)
  (vla-put-layer (vlax-ename->vla-object line3San) LayerName)
  (vla-put-layer (vlax-ename->vla-object texts1San) LayerName)
  (vla-put-layer (vlax-ename->vla-object texts2San) LayerName)
  (vla-put-layer (vlax-ename->vla-object texts3San) LayerName)
  (setq acceptableSan (strcase (getstring "\nIs the shape acceptable now? ([Y/N]): ")))
  (while  (or (equal acceptableSan "N")(equal acceptableSan "n"))
     (delete-and-redraw-shape blocknameSan)
    (vl-load-com)
    (setq docSan (vla-get-activedocument (vlax-get-acad-object)))
    (setq mspaceSan (vla-get-modelspace docSan))
    (setq ttSan 0)
    ...
    (setq endpoint4San (list (+ prevXSan 0.16) (- newYSan 0.12) prevZSan))
    (setq endpoint5San (list (+ prevXSan 0.25 ) (- newYSan 0.12) prevZSan))
    (setq midpointSan (polar endpoint4San (DTR 135) (/ (distance newAptSan endpoint1San) 2.0) ))
    (setq midpoint1San (polar endpoint5San (DTR 45) (/ (distance newAptSan endpoint1San) 2.0) ))
  ...
  (prompt "\nSelect Objects for Make Block")
  (setq selectionSan (ssget))
  (while (or (equal selectionSan nil) (< (sslength selectionSan) 3))
    (princ "Please select at least 3 objects.")
    (setq selectionSan (ssget))
  )

  (command-s "._-block" blocknameSan text1poSan selectionSan "")
  (setq blockrefSan (entlast))
  (vla-put-layer (vlax-ename->vla-object blockrefSan) LayerName)
  (vla-put-color (vlax-ename->vla-object blockrefSan) 2)
  ; Insert the block at the specified scale
  (setq scaleSan (* htSan 10)) ; Set the desired scale factor here
  (command "._insert" blocknameSan text1poSan scaleSan scaleSan "")
  (setq inrefSan (entlast))
  (vla-put-layer (vlax-ename->vla-object inrefSan) LayerName)
  

  (princ "Block Created Successfully.")
)
;;=======================================================================================================================
;;===========create a block for a Sanjaghi that is in both side in 90 Degree=============================================
;;=======================================================================================================================
...
;;=======================================================================================================================
;;===========create a block for a rebars with 90 degree bends base on overlap shape======================================
;;=======================================================================================================================
(defun create-90*rebar-block ( tool1-90r tool2-90r tool3-90r over-90r kham-90r ht-90r blockname-90r)
  (vl-load-com)
  (setq doc (vla-get-activedocument (vlax-get-acad-object)))
  (setq mspace (vla-get-modelspace doc))
  (initget (+ 1 2 4))
   (setq apt90r (getpoint "\nInsertion Point For Block shape: "))
  ; Calculate the new coordinates for creating lines
  (setq prevX-90r (car apt90r))
  (setq prevY-90r (cadr apt90r))
  (setq prevZ-90r (caddr apt90r))
  
  (setq apt-90r (list prevX-90r prevY-90r prevZ-90r))
  (setq endpoint-90r (list (+ 0.50 prevX-90r) prevY-90r prevZ-90r))
  (setq endpoint1-90r (list prevX-90r (- prevY-90r 0.1) prevZ-90r))
  (setq endpoint2-90r (list (+ 0.40 prevX-90r) (+ prevY-90r 0.08) prevZ-90r))
  (setq endpoint3-90r (list (+ 0.90 prevX-90r) (+ prevY-90r 0.08) prevZ-90r))
  (setq endpoint4-90r (list (+ 0.80 prevX-90r) (+ prevY-90r 0.16) prevZ-90r))
  (setq endpoint5-90r (list (+ 1.2 prevX-90r) (+ prevY-90r 0.16) prevZ-90r))
  (setq endpoint6-90r (list (+ 1.2 prevX-90r) (- prevY-90r 0.1) prevZ-90r))
  ; Create lines and fillets
  (command-s "._line" apt-90r endpoint-90r "" "" "")
  (setq line1-90r (entlast))
  (command-s "._line" apt-90r "<90" endpoint1-90r "" "" "")
  (setq line2-90r (entlast))
  (command-s "._line" endpoint2-90r "<180" endpoint3-90r "" "" "")
  (setq line3-90r (entlast))
  (command-s "._line" endpoint4-90r "<180" endpoint5-90r "" "" "")
  (setq line4-90r (entlast))
  (command-s "._line" endpoint5-90r "<180" endpoint6-90r "" "" "")
  (setq line5-90r (entlast))
  (command "._fillet" line1-90r "Radius" 0.02 "" line2-90r)
  (command "._fillet" line4-90r "Radius" 0.02 "" line5-90r)
 
  
  ; Helper function to convert degrees to radians
  (defun DtR-90r (d) (* pi (/ d 180.0)))
  
  ; Calculate additional points and midpoints
  (setq endpoint4-90r (list prevX-90r (- prevY-90r 0.12) prevZ-90r))
  (setq endpoint5-90r (list (+ prevX-90r 1.12) (- prevY-90r 0.12) prevZ-90r))
  (setq midpoint-90r (polar endpoint4-90r (DTR-90r 90) (/ (distance apt-90r endpoint1-90r) 2.0)))
  (setq midpoint1-90r (polar endpoint5-90r (DTR-90r 90) (/ (distance apt-90r endpoint1-90r) 2.0)))
  
  ; Define the position for text1
  (setq text1po-90r (list (+ 0.20 prevX-90r) (+ prevY-90r (* (/ ht-90r 15) 2)) prevZ-90r))
  (setq text2po-90r (list (+ 0.40 prevX-90r) (+ (+ prevY-90r 0.08) (* (/ ht-90r 15) 2)) prevZ-90r));overlap 1
  (setq text30po-90r (list (+ 0.80 prevX-90r) (+ (+ prevY-90r 0.16) (* (/ ht-90r 15) 2)) prevZ-90r));overlap 2
  (setq text3po-90r (list (+ 1 prevX-90r) (+ (+ prevY-90r 0.16) (* (/ ht-90r 15) 2)) prevZ-90r));text 3
  (setq text20po-90r (list (+ 0.60 prevX-90r) (+ (+ prevY-90r 0.08) (* (/ ht-90r 15) 2)) prevZ-90r));text 2
  ; Set the text style and create text entities
  (command-s "-style" "khayyam" "@Arial unicode MS" ht-90r 0.85 0 "N" "N")
  (setq text1-90r (rtos tool1-90r 2 2))
  (setq text11-90r (rtos tool2-90r 2 2))
  (setq text12-90r (rtos tool3-90r 2 2))
  (setq text2-90r (rtos kham-90r 2 2))
  (setq text3-90r (rtos kham-90r 2 2))
  (setq text4-90r (rtos over-90r 2 2))
  (setq thetext1-90r (vla-AddText mspace text1-90r (vlax-3d-point text1po-90r) 0.05))
  (vla-put-layer thetext1-90r LayerName)
  (setq texts1-90r (entlast))
  (setq thetext11-90r (vla-AddText mspace text4-90r (vlax-3d-point text2po-90r) 0.03))
  (vla-put-layer thetext11-90r LayerName)
  (setq thetext12-90r (vla-AddText mspace text12-90r (vlax-3d-point text3po-90r) 0.05))
  (vla-put-layer thetext12-90r LayerName)
  
  (setq thetext2-90r (vla-AddText mspace text2-90r (vlax-3d-point midpoint-90r) 0.03))
  (vla-put-layer thetext2-90r LayerName)
  (setq texts2-90r (entlast))
  (setq thetext3-90r (vla-AddText mspace text3-90r (vlax-3d-point midpoint1-90r) 0.03))
  (vla-put-layer thetext3-90r LayerName)
  (setq texts3-90r (entlast))
  (setq thetext4-90r (vla-AddText mspace text4-90r (vlax-3d-point text30po-90r) 0.03))
  (vla-put-layer thetext4-90r LayerName)
  (setq texts4-90r (entlast))
  (setq thetext5-90r (vla-AddText mspace text11-90r (vlax-3d-point text20po-90r) 0.05))
  (vla-put-layer thetext5-90r LayerName)
  ; Assign layers to entities
  (vla-put-layer (vlax-ename->vla-object line1-90r) LayerName)
  (vla-put-layer (vlax-ename->vla-object line2-90r) LayerName)
  (vla-put-layer (vlax-ename->vla-object line3-90r) LayerName)
  (vla-put-layer (vlax-ename->vla-object line4-90r) LayerName)
  (vla-put-layer (vlax-ename->vla-object line5-90r) LayerName)
  (vla-put-layer (vlax-ename->vla-object texts1-90r) LayerName)
  
  (vla-put-layer (vlax-ename->vla-object texts2-90r) LayerName)
  (vla-put-layer (vlax-ename->vla-object texts3-90r) LayerName)
  ; Check if the shape is acceptable
  (setq acceptable (getstring "\nIs the shape acceptable? [Y/N]: "))
  
  ; If the shape is not acceptable, delete and redraw it
  (while  (or (equal acceptable "N")(equal acceptable "n"))
    (delete-and-redraw-shape blockname)
     (vl-load-com)
    (setq doc (vla-get-activedocument (vlax-get-acad-object)))
  ...
  ; Prompt for object selection for block creation
  (prompt "\nSelect Objects for Make Block")
  (setq selection-90r nil)
  
  ; Validate the selection to have at least 3 objects
  (while (or (equal selection-90r nil) (< (sslength selection-90r) 3))
    (setq selection-90r (ssget))
    (princ "Please select at least 3 objects.")
  )

  ; Create block using selected objects
  (command-s "._-block" blockname-90r text1po-90r selection-90r "")
  (setq blockref-90r (entlast))
  (vla-put-layer (vlax-ename->vla-object blockref-90r) LayerName)
  
  ; Prompt for scale input
  (setq scale-90r (* ht-90r 10))
  
  ; Insert the block with the specified scale
  (command "._insert" blockname-90r text1po-90r scale-90r scale-90r "")
  (setq inref-90r (entlast))
  (vla-put-layer (vlax-ename->vla-object inref-90r) LayerName)
)
;;=======================================================================================================================
;;===========create a block for a rebars without bends base on overlap shape=============================================
;;=======================================================================================================================
(defun create-90*rebarwb-block ( tool90br1 tool90br2 tool90br3 over90br ht blockname)
  (vl-load-com)
  (setq doc (vla-get-activedocument (vlax-get-acad-object)))
  (setq mspace (vla-get-modelspace doc))
  (initget (+ 1 2 4))
  (setq apt90br (getpoint "\nInsertion Point For Block shape: "))
  ; Calculate the new coordinates for creating lines
  (setq prevX90br (car apt90br))
  (setq prevY90br (cadr apt90br))
  (setq prevZ (caddr apt90br))
  
  (setq newApt90br (list prevX90br prevY90br prevZ))
  (setq endpoint90br (list (+ 0.50 prevX90br) prevY90br prevZ))
  (setq endpoint90br1 (list prevX90br (- prevY90br 0.1) prevZ))
  (setq endpoint90br2 (list (+ 0.40 prevX90br) (+ prevY90br 0.08) prevZ))
  (setq endpoint90br3 (list (+ 0.90 prevX90br) (+ prevY90br 0.08) prevZ))
  (setq endpoint90br4 (list (+ 0.80 prevX90br) (+ prevY90br 0.16) prevZ))
  (setq endpoint90br5 (list (+ 1.2 prevX90br) (+ prevY90br 0.16) prevZ))
  
  ; Create lines and fillets
  (command-s "._line" newApt90br endpoint90br "" "" "")
  (setq line90br1 (entlast))
 
  (command-s "._line" endpoint90br2 "<180" endpoint90br3 "" "" "")
  (setq line90br3 (entlast))
  (command-s "._line" endpoint90br4 "<180" endpoint90br5 "" "" "")
  (setq line90br4 (entlast))
  ...
  ; Define the position for text1
  (setq text90br1po (list (+ 0.20 prevX90br) (+ prevY90br (* (/ ht 15) 2)) prevZ)); first text90br
  (setq text90br2po (list (+ 0.40 prevX90br) (+ (+ prevY90br 0.08) (* (/ ht 15) 2)) prevZ)); overlap text90br
  (setq text90br21po (list (+ 0.65 prevX90br) (+ (+ prevY90br 0.08) (* (/ ht 15) 2)) prevZ)); second text90br
   (setq text90br3po (list (+ 0.80 prevX90br) (+ (+ prevY90br 0.16) (* (/ ht 15) 2)) prevZ)); overlap
  (setq text90br4po (list (+ 1 prevX90br) (+ (+ prevY90br 0.16) (* (/ ht 15) 2)) prevZ)); thirth text90br
  ; Set the text90br style and create text90br entities
  (command-s "-style" "khayyam" "@Arial unicode MS" ht 0.85 0 "N" "N")
  (setq text90br1 (rtos tool90br1 2 2))
  (setq text90br11 (rtos tool90br2 2 2))
  (setq text90br12 (rtos tool90br3 2 2))
  (setq text90br3 (rtos over90br 2 2))
  
  (setq thetext90br1 (vla-AddText mspace text90br1 (vlax-3d-point text90br1po) 0.05))
  (vla-put-layer thetext90br1 LayerName)
  (setq text90brs1 (entlast))
   (setq thetext90br11 (vla-AddText mspace text90br11 (vlax-3d-point text90br21po) 0.05))
  (vla-put-layer thetext90br11 LayerName)
  (setq thetext90br12 (vla-AddText mspace text90br12 (vlax-3d-point text90br4po) 0.05))
  (vla-put-layer thetext90br12 LayerName)
  (setq thetext90br3 (vla-AddText mspace text90br3 (vlax-3d-point text90br3po) 0.03))
  (vla-put-layer thetext90br3 LayerName)
  (setq thetext90br4 (vla-AddText mspace text90br3 (vlax-3d-point text90br2po) 0.03))
  (vla-put-layer thetext90br4 LayerName)
  ; Assign layers to entities
  (vla-put-layer (vlax-ename->vla-object line90br1) LayerName)
  (vla-put-layer (vlax-ename->vla-object line90br3) LayerName)
  (vla-put-layer (vlax-ename->vla-object line90br4) LayerName)
 
  (vla-put-layer (vlax-ename->vla-object text90brs1) LayerName)
  ; Check if the shape is acceptable
  (setq acceptable (strcase (getstring "\nIs the shape acceptable? ([Y/N]): ")))
  ...
  ; Create block using selected objects
  (command-s "._-block" blockname text90br1po selection "")
  (setq blockref (entlast))
  (vla-put-layer (vlax-ename->vla-object blockref) LayerName)
  
  ; Prompt for scale input
  (setq scale90br (* ht 10))
  
  ; Insert the block with the specified scale90br
  (command "._insert" blockname text90br1po scale90br scale90br "")
  (setq inref (entlast))
  (vla-put-layer (vlax-ename->vla-object inref) LayerName)
)
;;=======================================================================================================================
;;=======================================================================================================================
;;=======================================================================================================================
(defun create-90*rwb-block ( tool1 ht blockname)
  (vl-load-com)
  (setq doc (vla-get-activedocument (vlax-get-acad-object)))
  (setq mspace (vla-get-modelspace doc))
  (setq apt90rwb (getpoint "\nInsertion Point For Block shape: "))

  ; Calculate the new coordinates for creating lines
  (setq prevX90rwb (car apt90rwb))
  (setq prevY90rwb (cadr apt90rwb))
  (setq prevZ (caddr apt90rwb))
  
  (setq newApt90rwb (list prevX90rwb prevY90rwb prevZ))
  (setq endpoint90rwb (list (+ 0.50 prevX90rwb) prevY90rwb prevZ))
  
  ; Create lines and fillets
  (command-s "._line" newApt90rwb endpoint90rwb "" "" "")
  (setq line90rwb1 (entlast))
 
  ; Define the position for text90br1
  (setq text90rwb1po (list (+ 0.20 prevX90rwb) (+ prevY90rwb (* (/ ht 15) 2)) prevZ))
  
  (command-s "-style" "khayyam" "@Arial unicode MS" ht 0.85 0 "N" "N")
  (setq text90rwb1 (rtos tool1 2 2))
  
  (setq thetext90rwb1 (vla-AddText mspace text90rwb1 (vlax-3d-point text90rwb1po) 0.05))
  (vla-put-layer thetext90rwb1 LayerName)
  (setq text90rwbs1 (entlast))
  
  ; Assign layers to entities
  (vla-put-layer (vlax-ename->vla-object line90rwb1) LayerName)
  
  (vla-put-layer (vlax-ename->vla-object text90rwbs1) LayerName)
  ; Check if the shape is acceptable
  (setq acceptable (strcase (getstring "\nIs the shape acceptable? (Y/N): ")))
  ...
  ; Validate the selection to have at least 3 objects
  (while (or (equal selection nil) (< (sslength selection) 2))
    (setq selection (ssget))
    (princ "Please select at least 2 objects.")
  )

  ; Create block using selected objects
  (command-s "._-block" blockname text90rwb1po selection "")
  (setq blockref (entlast))
  (setq blockrefe (entget blockref))
  (vla-put-layer (vlax-ename->vla-object blockref) LayerName)
  
  ; Prompt for scale input
  (setq scale90rwb (* ht 10))
  
  ; Insert the block with the specified scale90rwb
  (command "._insert" blockname text90rwb1po scale90rwb scale90rwb "")
  (setq inref (entlast))
  (setq inrefr (entget inref))
  (vla-put-layer (vlax-ename->vla-object inref) LayerName)
)
;;=======================================================================================================================
...
(defun put-text ()
    ...
    (initget (+ 1 2 4) "DistScale d D" ) ;; prohibited user for Enter 0,"", negative value
    ;; Prompt the user to enter a scale number for division on the final AddSum
    (setq scaledim (getreal "put scale of drawing 1/:[DistScale/Number]"))
      (if  (or (equal scaledim "Dist")(equal scaledim "D")(equal scaledim "d")(equal scaledim "dist")(equal scaledim "DistScale"))
        (progn
          (setq scaledim (getdist))
          (setq tdi (getreal (strcat "\nequal '"(rtos scaledim 2 2)"' actual length:")))
          (setq scaledim (/ scaledim tdi))
        )
      )
  ...
  (if (or (equal mainrebar "main") (equal mainrebar "m") (equal mainrebar "M") (equal mainrebar "Main") (equal mainrebar "MAIN"))
    (progn   
      ...
      (setq em 1)
        (while ( > em 0)
          ...
          (if (= length-choice 0)
            (progn
              (setq chhh (strcase (getstring "Select (s) or distance(d) [S/D]")))
              (while (or (equal chhh "") (equal chhh nil)) (setq chhh (strcase (getstring "Select (s) or distance(d) [S/D]"))))
              (if (equal chhh "S") (progn (sbe)(setq length-choice totalLen )) )
              (if (equal chhh "D") (progn (eza)(setq length-choice AddSum )) )
                  ...
                  (if (> length-choice 12) 
                      (progn
                          ...
                          (setq mspace (vla-get-modelspace 
                            ...
                            (setq ht (getreal "\nHeight : "))
                          (if (or (equal ht nil)(equal ht "")) (setq ht 0.2))
                          ...
                          (setq prevX (car apt))
                            ...
                            (setq thetextover (vla-AddText mspace finaltextover 
                                                          (vlax-3d-point newApt) 0.2))
                          (vla-put-layer thetextover LayerName )
                             (if (or(equal bend "b") (equal bend "B"))
                         (progn 
                           (setq prevX (car apt))
                          (setq prevY (cadr apt))
                          (setq prevZ (caddr apt))
                          ...
                          ; Set the offset distance for the new text below the previous text
                          (setq offset (* ht -8))  ; Adjust this value as needed
                      ...
                      (create-90*rebar-block  (- 12 khamm) 12 (- remm khamm) overlap-length khamm ht blockname))
                       (progn 
                           (setq prevX (car apt))
                          (setq prevY (cadr apt))
                          (setq prevZ (caddr apt))
                      ...
                      (create-90*rebarwb-block  max-length 12 remm overlap-length ht blockname))
                       )
                                  ...
                                  (alert (strcat "this is string: " " "finaltext) )
                                    ...
                                    (while (tblsearch "BLOCK" blockname)
                                      (progn
                                        ...
                                        (if (< khamm 0.10) (setq khamm 0.1))
                                        (create-90*Sanjaghi-block newApt (- totalLen (* khamm 2)) khamm ht blockname)
                          
                        )
                        (progn
                            
                                 
                                  (alert  (strcat (rtos length-choice 2 2 ) )   )
                                  ...
                                  (alert (strcat "this is string: " " "finaltext) )
                                    ...
                                    (while (tblsearch "BLOCK" blockname)
                                      (progn
                                        ...
                                        (create-90*rwb-block   totalLen ht blockname)
                        )
                      )
                    )
                  ) 
                ...
                (progn 
                      ...
                      (if 
                        ...
                        (progn 
                          (setq kham (* dia  12))
                            (setq khamm (/ kham 1000.))
                          (setq totalLen (+ length-choice (* khamm 2)))
                            (initget (+ 1 4))
    ...
    (progn
        ...
        (while ( > e1 0)
            ...
            (setq shapes (strcase(getstring t "choose the shape of elements khamoot(k)/135*Sanjaghi(S)/90*Sanjaghi(N)/90*135Sanjaghi(NS) [K/S/N/NS] ")))
            )
          ...
          (if (or (equal shapes "Khamoot")  (equal shapes "k") (equal shapes "K") (equal shapes "khamoot"))
            (progn
              ...
              (setq ht (getreal "\nHeight : "))
               (if (or (equal ht nil)(equal ht "")) (setq ht 0.2))             ;;(vla-put-layer finaltext  "steels")
              (command-s "-style" "khayyam" "@Arial unicode MS" ht 0.85 0 "N" "N")
              (setq thetext (vla-AddText mspace finaltext 
                                                (vlax-3d-point apt) ht))
          ...
          (if (or (equal shapes "s") (equal shapes "S") )
            (progn
               ...
               (defun calculate-135*Sanjaghi-length ()
                ...
                (if (< widthsn 0.10) (setq widthsn 0.10))
              ...
              (princ (strcat "\nStirrup Length: " (rtos Sanjaghi35-length 2 2) " m\n"))
                          ...
                          ; Set the offset distance for the new text below the previous text
                          (setq offset (* ht -3.5))  ; Adjust this value as needed

                          ; Calculate the new Y-coordinate for the insertion point
                          (setq newY (+ prevY offset))

                          ; Create the new insertion point with the updated coordinates
                          (setq newApt (list (+ prevX (* ht 4)) newY prevZ))
                        (setq blockname (strcat  nameOfObject   (rtos position 2 0)) )
                        (while (tblsearch "BLOCK" blockname)
                          (progn
                            (setq nnn 1)
      ...
      (if (setq eset (ssget))
        (progn
          ...
          (while (< cntr (sslength eset))
            ...
            ;; Update the total length and increment the counter
            (setq totalLen (+ totalLen (/ len scale)))
            (setq cntr (+ cntr 1))
          )
        )
      )
  ...
  (setq weight-list  '(("Ø8"  "0.617") ("Ø10"  "0.617") ("Ø12"  "0.888") ("Ø14"  "1.21") ("Ø16"  "1.58") ("Ø18" "2.00") ("Ø20" "2.47") ("Ø22" "2.98") ("Ø25" "3.85") ("Ø28"  "4.83") ("Ø30"  "5.55") ("Ø32"  "6.31")))
    (foreach line lines
      ...
      (if s
        ...
        (setq unit-weight (cdr (assoc pose weight-list)))
        (setq totalWeigth (* totalqty (atof numbe) (atof leng) (atof  (car unit-weight))))
        (setq total-Lnumber (* totalqty (atof numbe) (atof leng)))
        (setq sumw (cons  totalWeigth sumw))
        (setq tab (list first pose numbe diame ate leng totalle (rtos totalqty 2 0) (rtos total-Lnumber 2 2) (rtos totalWeigth 2 2) 8th))
...
(defun put-text ()
    ...
    (initget (+ 1 2 4) "DistScale d D" ) ;; prohibited user for Enter 0,"", negative value
    ;; Prompt the user to enter a scale number for division on the final AddSum
    (setq scale (getreal "put scale of drawing 1/:[DistScale/Number]"))
      (if  (or (equal scale "Dist")(equal scale "D")(equal scale "d")(equal scale "dist")(equal scale "DistScale"))
        (progn
          (setq scale (getdist))
          (setq tdi (getreal (strcat "\nequal '"(rtos scale 2 2)"' actual length:")))
          (setq scale (/ scale tdi))
        )
      )
